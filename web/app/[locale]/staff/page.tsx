...
async function getStaff() {
  return client.fetch(`*[_type == "staffMember"] | order(order asc) { _id, name, role, photo, bio, email }`)
}
  ...
  return (
    <main>
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        {staff.length === 0 && <p className="text-gray-400 text-sm">{t('empty')}</p>}
          ...
          {staff.map((member: any) => (
            <div key={member._id} className="bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 hover:border-gold transition-colors">
              <div className="relative h-52 bg-gray-100">
                {member.photo ? (
                  <Image src={urlFor(member.photo).width(400).height(300).fit('crop').url()} alt={member.name} fill className="object-cover" />
                ) : (
              ...
              <div className="p-5">
                ...
                {member.email && <a href={`mailto:${member.email}`} className="inline-block mt-3 text-xs text-navy hover:text-gold underline transition-colors">{member.email}</a>}
              </div>
            </div>
          ))}
        </div>
